	...
	public static void handleTable(LootTable table, ResourceLocation name) {
		...
		if (name.getResourceDomain().equals(JurassiCraft.MODID)) {
			if (name.getResourcePath().equals(Loot.VISITOR_GROUND_STORAGE.getResourcePath())) {
				...
				LootEntry fossilPart = Loot.entry(ItemHandler.FOSSILS.get("tooth")).weight(1).function(DINOSAUR_DATA)
						.count(1, 3).build();
				fossilParts.add(fossilPart);
				// }
				table.addPool(Loot.pool("items").rolls(1, 2)
						.entries(fossilParts.toArray(new LootEntry[fossilParts.size()])).build());
			}
	...
	public static class DinosaurData extends LootFunction {
		...
		public ItemStack apply(ItemStack stack, Random rand, LootContext context) {
			...
			if (stack.getItem() instanceof FossilItem) {
				...
				List<Dinosaur> boneTypeDinos = s.fossilDinosaurs.get("tooth");
